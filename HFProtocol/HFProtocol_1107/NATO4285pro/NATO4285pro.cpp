...
CNATO4285pro::CNATO4285pro(void)
{
	pPreambleSym = NULL;
	pProbleSym = NULL;	
	pBufInter = NULL;
...
void CNATO4285pro::PSK_map(short style, short M, float I_map[],float Q_map[])
{
	switch(M)
	{
	case 2:
...
int CNATO4285pro::PSK_MOD( short data[], short data_len, short M,	 Ipp32fc *out)
{
	int i;
	float *I_map,*Q_map;
	I_map = new float[M];
...
void CNATO4285pro::Preamble_Gen()
{
	UWleng = 80;

	pPreambleSym = ippsMalloc_32fc(80);
	PSK_MOD(preamble_4285,80,2,pPreambleSym);
	UWEnergy = 1;
...
////////////////////////////////////////////////////////////////
float CNATO4285pro::do_fir(float *base, int cur, float *coeff, int len)
{
	int c=0, i;
	float sum=0;
...
//  N阶根升余弦滤波器
//////////////////////////
void CNATO4285pro::SRcos_Filter(int N, float RollOff, float fBaudRate, float fSampleRate, float *fMF_Coef)
{
...
void CNATO4285pro::CrossConvCof(Ipp32fc *pSrcA,Ipp32fc *pSrcB,Ipp16s nLeng,Ipp32f bEnergy,Ipp32f *cConv)
{
...
/************************************************************************/
void CNATO4285pro::Burst_detect_ini(int allLen,int convLen,int deciP,int mproBegin)
{
...
void CNATO4285pro::Burst_detect_free()
{
	ippsFree(winBufConv);
	ippsFree(proBuf);
	ippsFree(delayBuf);
...
/************************************************************************/
void CNATO4285pro::Burst_detect(int proBegin,int proLen,Ipp32fc *UWsig,Ipp32f UWEnergy, Ipp16s convLen,Ipp16s deciP,
...
void CNATO4285pro::DownFre_ini(Ipp32f Insample,Ipp32f Outsample,Ipp32s nLeng,Ipp32f fre,int FFTLen,Ipp32f roll,Ipp32f Baud,Ipp16s P,int SrctapLen)
{
	pfre = fre/Outsample;
...
void CNATO4285pro::DownFre_free()
{
	ippsFree(pSrcTap);
	ippsFree(pDelayreal);
	ippsFree(pDelayimag);
...
void CNATO4285pro::DownFre(Ipp16s *pSrc,int nLeng,Ipp16s P,int FFTLen,Ipp32f TH1,Ipp32f TH2,Ipp32fc *pDst,int &DstLen)
{
	int i;
	int BurstNum;
	int BufHavePro = 0;
...
void CNATO4285pro::Demode_PSKrealFSE_ini(Ipp32f Insample,Ipp32f Outsample,Ipp32s nLeng,Ipp32f fre,Ipp16s P,int FFTLen,Ipp32f roll,Ipp32f Baud,int SrctapLen,
										 int mdataRate,int mFECType,int mInterType)
{
	...
	if(dataRate==75 ||dataRate==150 || dataRate==300 || dataRate==600)
	{
		if(InterType==1)
			increasment=1;
		else if(InterType==2)
...
void CNATO4285pro::Demode_PSKrealFSE_free()
{
	ippsFree(OneFrame);
	Burst_detect_free();
	DownFre_free();
...
void CNATO4285pro::Demode_PSKrealFSE(Ipp16s *pSrcDst,int nLeng,Ipp16s P,int FFTLen,Ipp32f TH1,Ipp32f TH2,
									 int &outLeng,Ipp8u *outbyte,int &byteLeng, HeadType4285 *headType,int &headNum)
{
	...
	DeHead4285(BurstPos,BurstNum,headType,headNum);
...
void CNATO4285pro::DeHead4285(Ipp16s *BurstPos,int BurstNum,HeadType4285 *headType,int &headNum)
{
	int i;
	...
	for (i=0;i<BurstNum;i++)
	{
		...
		if(InterType==0)
			headType[i].interType = std::string("NO");
		else if(InterType==1)
			headType[i].interType = std::string("SHORT");
		else if(InterType==2)
			headType[i].interType = std::string("LONG");
	}
	headNum = BurstNum;
}
/************************************************************************/
...
/************************************************************************/
void CNATO4285pro::Synchronization_PSK(Ipp32fc *FrameBuf,Ipp16s FrameLen,Ipp16s P,Ipp32fc *pUWsig,Ipp16s UWleng,int delayCma,
									  Ipp32fc *pDst,int &outleng,Ipp8u *outbyte,int &byteleng,int bBegin)
{
	...
	// 取出前导序列同步估计
	ParaEstimate(FrameBuf,80*P,P,pUWsig,mfre,pllphase,pEnergy);
...
//	outleng = outleng + moutleng;
	...
	// 解交织译码
	if((dataRate==75 || dataRate==150 || dataRate==300 || dataRate==600)&& FECType==1)// 有编码
	...
	else if(dataRate==2400 && FECType==1)
		MapFor4285(pJudge,judgelen,pBufInter,InterLength,3);// 8PSK
	// 无编码无交织情况
	else if(dataRate==1200 && FECType==0)
		MapFor4285(pJudge,judgelen,pBufInter,InterLength,1);// BPSK
	else if(dataRate==2400 && FECType==0)
		MapFor4285(pJudge,judgelen,pBufInter,InterLength,2);// QPSK
	else if(dataRate==3600 && FECType==0)
		MapFor4285(pJudge,judgelen,pBufInter,InterLength,3);// 8PSK
	else
		MapFor4285(pJudge,judgelen,pBufInter,InterLength,3);// 8PSK
...
void CNATO4285pro::EqualizationRLS_DFE_PLL_ini(Ipp32f wn, Ipp32f kesai,Ipp16s M_phase,Ipp32f pll_phase,
...
void CNATO4285pro::EqualizationRLS_DFE_PLL_free()
{
	m_RLS->RLS_free();
	if(m_RLS){
		delete m_RLS;
...
//  分数间隔判决反馈均衡
void CNATO4285pro::EqualizationRLS_DFE_PLL(Ipp32fc *pSrc,int nLeng,int P,int delay,Ipp32f mEnergy,Ipp32fc *UWsig,Ipp32fc *pDst,int &outLen,BOOL aid)
{
	int i;
	Ipp32fc xtemp;
	Ipp32f eph;
...
void CNATO4285pro::ParaEstimate(Ipp32fc *pSrc,int nLeng,int P,Ipp32fc *pUWsig,Ipp32f &fre,Ipp32f &phase,Ipp32f &energy)
{
...
void CNATO4285pro::Timing_ini()
{
	xi0=0;	xi1=0;	xi2=0;
	xq0=0;	xq1=0;	xq2=0;
	muk = 0;
}
void CNATO4285pro::Timing_estimate(Ipp32fc *pUWsig,Ipp16s UWleng,Ipp32fc *pSrc,Ipp16s P, Ipp32f &tau)
{
	Ipp16s i,j,k=0;
...
/*  定时同步，4倍采样                                                  */
/************************************************************************/
void CNATO4285pro::Timing_recover(Ipp32fc *pSrc,int nleng,Ipp32f tau,Ipp16s P,Ipp32fc *pDst,int &outleng)
{
	Ipp32f fltIdx;
	int mk,i,outn=0;
...
/************************************************************************/
float CNATO4285pro::sym_interp_i(float xi3, float u,bool ini)
{  
	float u2,u3;
	float y=0;
	if(ini)
...
/************************************************************************/
 /*  符号定时插值器 q
/************************************************************************/
float CNATO4285pro::sym_interp_q(float xq3, float u,bool ini)
{
	float u2,u3;
	float y=0;
	if(ini)
...
/* 数据辅助的LR频偏估计                                                  */
/************************************************************************/
void CNATO4285pro::Fre_Estimate_LR(Ipp32fc *pUWsig,Ipp32fc *pSrc,Ipp16s nleng,Ipp32f &fre)
{
	Ipp16s i,N=nleng/2;
...
void CNATO4285pro::Remove_fre(Ipp32fc *pSrc,Ipp32s nLeng, Ipp32f rFreq)
{
	Ipp32f temp;
	Ipp16s i;
...
void CNATO4285pro::Phase_VV(Ipp32fc *pSrc,Ipp32fc *pUWsig,Ipp16s len,Ipp32f &phase)
{
	Ipp16s i;
	Ipp32fc temp;
...
void CNATO4285pro::DecPSK(Ipp32fc pSrc,Ipp32fc &pDst)
{
...
void CNATO4285pro::DecToBit(Ipp8u pSrc,int M,Ipp8u *pDst,int &outLeng)
{
	int i;
	switch(M)
	{
...
void CNATO4285pro::SaveTobit_ini()
{
	byte_flag = 0x80;
	data_byte = 0;
	bit_num = 8;
...
void CNATO4285pro::SaveTobit(Ipp8u *pSrc,int nLeng,Ipp8u *outbyte,int &byteLeng,Ipp8s modutype)
{
	Ipp32s i,m,j;
	switch(modutype)
	{
...
/*   modutype:  1->BPSK
				...
				3->8PSK
/************************************************************************/
void CNATO4285pro::judge(Ipp32fc *pSrc,int nLeng,Ipp8u *pDst,Ipp8s modutype)
{
	Ipp32f *angle;
	Ipp32s i,m,j;
...
void CNATO4285pro::MapFor4285(Ipp8u *pSrc,int nLeng,Ipp8u *pDst,int &outLeng,Ipp8s modutype)
{
	int i;
	switch(modutype)
	{
