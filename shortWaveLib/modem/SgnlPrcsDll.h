...
const short SMT_NUM_MAX = SMT_16APSK + 1;
extern std::unordered_map<int, std::string> umap_input_uint8;
extern std::unordered_map<std::string, int> umap_input_string;



/* 信号调制类型对应名字 */
extern const char* g_SysModuName[];
...
// 均衡器类型 //
enum EqualizerType { EQUATYPE_NO, EQUATYPE_MCMA, EQUATYPE_MDFE };
...
// 获取信号频率 //
extern double GetSignalFreq(double dSignalFreq, DATA_CHANNEL_PARAM *pChanPara,
                            bool bInv = true);
// 获取信号数字频率 //
extern double GetDigitalFreq(double dSignalFreq, DATA_CHANNEL_PARAM *pChanPara,
                             bool bInv = true);
// 二次调制的AM解调//
