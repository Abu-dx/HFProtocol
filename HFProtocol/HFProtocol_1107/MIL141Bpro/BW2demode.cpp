...
void CBW2demode::PSK_map(short style, short M, float I_map[],float Q_map[])
{
	switch(M)
	{
	case 2:
...
int CBW2demode::PSK_MOD( short data[], short data_len, short M,Ipp32fc *out)
{
	int i;
	float *I_map,*Q_map;
	I_map = new float[M];
...
//  N阶根升余弦滤波器
//////////////////////////
void CBW2demode::SRcos_Filter(int N, float RollOff, float fBaudRate, float fSampleRate, float *fMF_Coef)
{
...
void CBW2demode::CrossConvCof(Ipp32fc *pSrcA,Ipp32fc *pSrcB,Ipp16s nLeng,Ipp32f bEnergy,Ipp32f *cConv)
{	
	Ipp32fc pMeanAB;
	Ipp32f pMeanA;
...
/************************************************************************/
void CBW2demode::Burst_detect_ini(int allLen,int convLen,int deciP,int mproBegin)
{
...
void CBW2demode::Burst_detect_free()
{
	ippsFree(proBuf);
	ippsFree(delayBuf);
	ippsFree(pConvAB);
...
/************************************************************************/
void CBW2demode::Burst_detect(int proBegin,int proLen,Ipp32fc *UWsig,Ipp32f UWEnergy, Ipp16s convLen,Ipp16s deciP,
...
void CBW2demode::DownFre_ini(Ipp32f Insample,Ipp32f Outsample,Ipp32s nLeng,Ipp32f fre,int FFTLen,Ipp32f roll,Ipp32f Baud,Ipp16s P,int SrctapLen)
{
...
void CBW2demode::DownFre(Ipp16s *pSrc,int nLeng,Ipp16s P,int FFTLen,Ipp32f TH1,Ipp32f TH2,Ipp32fc *pDst,int &DstLen)
{
	int i;
	int BurstNum;
	int BufHavePro = 0;
...
void CBW2demode::Demode_PSKrealFSE_ini(Ipp32f Insample,Ipp32f Outsample,Ipp32s nLeng,Ipp32f fre,Ipp16s P,int FFTLen,Ipp32f roll,Ipp32f Baud,int SrctapLen)
{
	...
	probe_sym = ippsMalloc_32fc(48);

	BW2preamble = ippsMalloc_32fc(64);
	PSK_MOD(preamble_BW2,64,8,BW2preamble);
...
void CBW2demode::Demode_PSKrealFSE_free()
{
	ippsFree(OneFrame);
	ippsFree(probe);
	ippsFree(probe_sym);
	ippsFree(BW2preamble);
	DownFre_free();
	Burst_detect_free();
...
void CBW2demode::Demode_PSKrealFSE(Ipp16s *pSrcDst,int nLeng,Ipp16s P,int FFTLen,Ipp32f TH1,Ipp32f TH2,Ipp16s *expout,
									int &outLeng,Ipp8u *outbyte,int &byteLeng,HeadType141B *headType,int &headNum)
{
	int i;
	int UWleng = m_Preambledetect->PreambleLen;
	...
	DeHead(BurstPos,BurstNum,headType,headNum);
	...
	while(pidx + (UWleng+5)*P<BufHavePro)
	{
		...
		else if (flag==1)// 找到前导进行同步估计
		{
		    ...
		    Synchronization(&proBuf[proBegin+pidx],OneFrameLen,P,m_Preambledetect->pPreambleSym,UWleng,5,OneFrame,TimeOutLen,&outbyte[byteLeng],mbyteleng,1);
			byteLeng = byteLeng +mbyteleng;
			pidx = pidx + OneFrameLen;
			flag = 2;
			OneFrameLen = (UWleng)*P; 
		}
		else if(flag==2)// 64个符号preamble
		{
			Synchronization(&proBuf[proBegin+pidx],64*P,P,BW2preamble,64,5,OneFrame,TimeOutLen,&outbyte[byteLeng],mbyteleng,2);
			byteLeng = byteLeng +mbyteleng;
			pidx = pidx + 64*P;
			flag = 3;
		}	
		...
		{
		    Synchronization(&proBuf[proBegin+pidx],OneFrameLen,P,m_Preambledetect->pPreambleSym,UWleng,5,OneFrame,TimeOutLen,&outbyte[byteLeng],mbyteleng,0);
			byteLeng = byteLeng +mbyteleng;
			pidx = pidx + OneFrameLen;
			if(BurstNum>0 && pidx<=BurstPos[pBurst] && BurstPos[pBurst]<=pidx+OneFrameLen)
				flag=4;
		}
		else if (flag==4 && BurstNum>0 && pidx<=BurstPos[pBurst] && BurstPos[pBurst]<=pidx+OneFrameLen)
		{
			TimeOutLen = 0;
			flag = 1;
			pidx = BurstPos[pBurst];
		...
		if (TimeOutLen>0)
		{
			...
			{
				expout[outLeng + 2*i]=Ipp16s(OneFrame[i].re*1000);
				expout[outLeng + 2*i+1]=Ipp16s(OneFrame[i].im*1000);
			}
...
void CBW2demode::DeHead(Ipp16s *BurstPos,int BurstNum,HeadType141B *headType,int &headNum)
{
	int i;
...
/************************************************************************/
void CBW2demode::Synchronization(Ipp32fc *FrameBuf,Ipp16s FrameLen,Ipp16s P,Ipp32fc *pUWsig,Ipp16s UWleng,int delayCma,
									  Ipp32fc *pDst,int &outleng,Ipp8u *outbyte,int &byteleng,int bBegin)
{
	...
	else if(bBegin==0)
	{
		...
		{
			EqualizationRLS_DFE_PLL(&pDownS[p],64,2,0,pEnergy,pUWsig,&pDst[outleng],moutlen,FALSE);		
			////  去扰
			for (i=0;i<32;i++)
			{
				k = probeNum*48+i;
			...
			PSK_MOD(probe,32,8,probe_sym);
			ippsConj_32fc_I(probe_sym,32);
			ippsMul_32fc(probe_sym,&pDst[outleng],pDownS,32);
			judge(pDownS,32,pJudge,4);	

			SaveTobit(pJudge,32,&outbyte[byteleng],mbyteleng,3);
			byteleng = byteleng + mbyteleng;
			...
			PSK_MOD(probe,16,8,probe_sym);
			// 利用信道探测序列均衡
			outleng = outleng + moutlen;
			EqualizationRLS_DFE_PLL(&pDownS[p+64],16*2,2,0,pEnergy,probe_sym,&pDst[outleng],moutlen,TRUE);

			ippsConj_32fc_I(probe_sym,16);
			ippsMul_32fc(probe_sym,&pDst[outleng],pDownS,16);
			judge(pDownS,16,pJudge,4);
	...
	else if(bBegin==2) // 同步跟踪 64个符号
	{	
		ippsSampleDown_32fc(FrameBuf,FrameLen,pDownS,&pDownSLen,2,&pPhase);
		Remove_fre(pDownS,pDownSLen,mfre);	
		EqualizationRLS_DFE_PLL(pDownS,pDownSLen,2,0,pEnergy,BW2preamble,pDst,outleng,TRUE);	// 辅助
	}
	ippsFree(pJudge);
	ippsFree(pDownS);
}
void CBW2demode::ParaEstimate(Ipp32fc *pSrc,int nLeng,int P,Ipp32fc *pUWsig,Ipp32f &fre,Ipp32f &phase,Ipp32f &energy)
{
	Ipp32fc *pTemp = ippsMalloc_32fc(nLeng);
	int outLen;

	Timing_ini();
	Timing_estimate(pUWsig,240,pSrc,P,mtau);
	Timing_recover(pSrc,nLeng,mtau,P,pTemp,outLen);
	Fre_Estimate_LR(pUWsig,pTemp,240,fre);

	frephase = 0;
	Remove_fre(pTemp,240, fre);
	Phase_VV(pTemp,pUWsig,240,phase);

	Ipp32f pEnergy = 0;
	int i;
	for(i=0;i<240;i++)
		pEnergy = pEnergy + sqrt(pTemp[i].re*pTemp[i].re+pTemp[i].im*pTemp[i].im)/240;
	energy = pEnergy*2;

	ippsFree(pTemp);
	frephase = 0;
}
void CBW2demode::EqualizationRLS_DFE_PLL_ini(Ipp32f wn, Ipp32f kesai,Ipp16s M_phase,Ipp32f pll_phase,
...
void CBW2demode::EqualizationRLS_DFE_PLL_free()
{
	m_RLS->RLS_free();
	if(m_RLS){
		delete m_RLS;
...
//  分数间隔判决反馈均衡，针对PSK
void CBW2demode::EqualizationRLS_DFE_PLL(Ipp32fc *pSrc,int nLeng,int P,int delay,Ipp32f mEnergy,Ipp32fc *UWsig,Ipp32fc *pDst,int &outLen,BOOL aid)
{
	int i;
	Ipp32fc xtemp;
	Ipp32f eph;
...
void CBW2demode::DecToBit(Ipp8u pSrc,int M,Ipp8u *pDst,int &outLeng)
{
	int i;
	switch(M)
	{
...
/* 数据辅助的LR频偏估计                                                  */
/************************************************************************/
void CBW2demode::Fre_Estimate_LR(Ipp32fc *pUWsig,Ipp32fc *pSrc,Ipp16s nleng,Ipp32f &fre)
{
	Ipp16s i,N=nleng/2;
...
void CBW2demode::Remove_fre(Ipp32fc *pSrc,Ipp32s nLeng, Ipp32f rFreq)
{
	Ipp32f temp;
	Ipp16s i;
...
void CBW2demode::Phase_VV(Ipp32fc *pSrc,Ipp32fc *pUWsig,Ipp16s len,Ipp32f &phase)
{
	Ipp16s i;
	Ipp32fc temp;
...
//  收敛到0相位                                            
/////////////////////////////////////////////////////////////////////////////
void CBW2demode::dfpll_phase_ini(Ipp32f wn, Ipp32f kesai,Ipp16s M_phase)
{
	wn_p=wn;
	kesai_p=kesai;
	ko_p=1.0;
...
void CBW2demode::dfpll_phase(Ipp32fc *pSrc,Ipp32s nLeng,Ipp32f pll_phase)
{
	float e;
	Ipp32f arg,temp;
	pllphase=pll_phase;
...
void CBW2demode::Timing_estimate(Ipp32fc *pUWsig,Ipp16s UWleng,Ipp32fc *pSrc,Ipp16s P, Ipp32f &tau)
{
	Ipp16s i,j,k=0;
...
/*  定时同步，4倍采样                                                  */
/************************************************************************/
void CBW2demode::Timing_recover(Ipp32fc *pSrc,int nleng,Ipp32f tau,Ipp16s P,Ipp32fc *pDst,int &outleng)
{
	Ipp32f fltIdx;
	int mk,i,outn=0;
...
/************************************************************************/
float CBW2demode::sym_interp_i(float xi3, float u,bool ini)
{  
	float u2,u3;
	float y=0;
	if(ini)
...
/************************************************************************/
 /*  符号定时插值器 q
/************************************************************************/
float CBW2demode::sym_interp_q(float xq3, float u,bool ini)
{
	float u2,u3;
	float y=0;
	if(ini)
...
void CBW2demode::SaveTobit(Ipp8u *pSrc,int nLeng,Ipp8u *outbyte,int &byteLeng,Ipp8s modutype)
{
	Ipp32s i,m,j;
	switch(modutype)
	{
...
/*   modutype:  1->BPSK
				...
				3->8PSK
/************************************************************************/
void CBW2demode::judge(Ipp32fc *pSrc,Ipp16s nLeng,Ipp8u *pDst,Ipp8s modutype)
{
	Ipp32f *angle;
	Ipp32s i;
