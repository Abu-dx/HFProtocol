...
CSignalDemodProbe::~CSignalDemodProbe(void) {
  ...
  if (NULL != m_pNormalizedDemodBuf) {
    delete[] m_pNormalizedDemodBuf;
    m_pNormalizedDemodBuf = NULL;
  }
...
DWORD CSignalDemodProbe::InitDemodParam(double dSamplingFreq,
                                        double dSymbolRate, double dFreqShift,
                                        SHORT nModuType, bool bIsDecode) {
...
DWORD CSignalDemodProbe::InputData(double *pData, DWORD dwDataLen) {
  ...
  m_dwSylNum = 0;
  memset(m_pDemodBuf, 0, m_dwDemLen * sizeof(LONG));
  switch (m_nModuType) {
  case SMT_2ASK:
  {
...
DWORD CSignalDemodProbe::InitParam_AMS() {
  ...
  if (!m_pFilter->IinitialLPFPara(nLPFType, dPasswidth, dStopwidth, dPassRipple,
                                  dStopRipple)) {
...
DWORD CSignalDemodProbe::InitParam_FSK() {
  ...
  memset(m_pFSKDelay, 0, m_nFSKDelay * sizeof(double));
  ...
  if (!m_pFilter->IinitialLPFPara(nLPFType, dPasswidth, dStopwidth, dPassRipple,
                                  dStopRipple)) {
    LONG dwErrorID = ERROR_DEMOD_PARAM;
  }
...
void CSignalDemodProbe::GetSymbolLoop(double *pCoef, SHORT M) {
  ...
  w = (2.0 * PI) * pCoef[1] * dRatio *
      ((1.0 + 4.0 * pCoef[0] * pCoef[0]) / (8.0 * pCoef[0]));
...
DWORD CSignalDemodProbe::Demod_AM(double *pData, DWORD dwDataLen) {
  if (m_pNormalizedDemodBuf != NULL && m_dwSgnlLen < dwDataLen / 2) {
    delete[] m_pNormalizedDemodBuf;
    m_pNormalizedDemodBuf = NULL;
  }

  if (m_pNormalizedDemodBuf == NULL) {
    m_dwSgnlLen = dwDataLen / 2;
    m_pNormalizedDemodBuf = new double[m_dwSgnlLen];
  }
  ...
  for (i = 0; i < dwDataLen / 2; i++) {
    dSignNormal +=
        pData[2 * i] * pData[2 * i] + pData[2 * i + 1] * pData[2 * i + 1];
  }
  ...
  ////求瞬时幅度////
  for (i = 0; i < dwDataLen / 2; i++) {
    m_pNormalizedDemodBuf[i] = sqrt(pData[2 * i] * pData[2 * i] +
                         pData[2 * i + 1] * pData[2 * i + 1]) -
                    1.25;
  }
  m_pFilter->Filter(m_pNormalizedDemodBuf, dwDataLen / 2);

  // 量化 //
  for (i = 0; i < dwDataLen / 2; i++) {
    m_pDemodBuf[i] = static_cast<short>(10000 * (m_pNormalizedDemodBuf[i]));
  }
...
DWORD CSignalDemodProbe::Demod_FM(double *pData, DWORD dwDataLen) {
  ...
  m_pNormalizedDemodBuf[0] = atan2(tempQ, tempI);

  ////求瞬时频率////
  for (i = 1; i < dwDataLen / 2; i++) {
    tempQ =
        pData[2 * i + 1] * pData[2 * i - 2] - pData[2 * i] * pData[2 * i - 1];
    tempI =
        pData[2 * i] * pData[2 * i - 2] + pData[2 * i + 1] * pData[2 * i - 1];

    m_pNormalizedDemodBuf[i] = atan2(tempQ, tempI);
  }
...
DWORD CSignalDemodProbe::Demod_SSB(double *pData, DWORD dwDataLen) {
  ...
  for (i = 0; i < dwDataLen / 2; i++) {
    m_pNormalizedDemodBuf[i] = pData[2 * i] / dSignNormal;
  }

  m_pFilter->Filter(m_pNormalizedDemodBuf, dwDataLen / 2);

  for (i = 0; i < dwDataLen / 2; i++) {
    m_pDemodBuf[i] = static_cast<LONG>(SHRT_MAX * m_pNormalizedDemodBuf[i]);
  }
...
DWORD CSignalDemodProbe::Demod_ASK(double *pData, DWORD dwDataLen) {
  ...
  if (m_pNormalizedDemodBuf != NULL && m_dwSgnlLen < m_nFSKDelay + dwSignLen) {
    delete[] m_pNormalizedDemodBuf;
    m_pNormalizedDemodBuf = NULL;
  }
  ...
  if (m_pNormalizedDemodBuf == NULL) {
    ...
    m_pNormalizedDemodBuf = new double[m_nFSKDelay + dwSignLen];
  }
  ...
  for (n = 0; n < m_nFSKDelay; n++) {
    m_pNormalizedDemodBuf[n] = m_pFSKDelay[n];
  }
  // 去直流分量 //
  double dSignNormal = 0.0;
  ...
  for (i = 0; i < dwSignLen; i++) {
    m_pNormalizedDemodBuf[m_nFSKDelay + i] = m_pDemodBuf[i] - dSignNormal;
  }
  // 保留当前一个符号数据 //
  for (n = 0; n < m_nFSKDelay; n++) {
    m_pFSKDelay[n] = m_pNormalizedDemodBuf[dwSignLen + n];
  }

  dwCounter = 0;
  for (i = 0; i < dwSignLen; i++) {
    dDataI = m_pNormalizedDemodBuf[i];
    dDataQ = m_pNormalizedDemodBuf[i + m_nFSKDelay];
...
DWORD CSignalDemodProbe::Demod_FSK(double *pData, DWORD dwDataLen) {
  ...
  for (i = 0; i < dwSignLen; i++) {
    m_pNormalizedDemodBuf[m_nFSKDelay + i] = m_pDemodBuf[i];
  }
  // 保留前一个符号数据 //
  for (n = 0; n < m_nFSKDelay; n++) {
    m_pFSKDelay[n] = m_pNormalizedDemodBuf[dwSignLen + n];
...
DWORD CSignalDemodProbe::Demod_MSK(double *pData, DWORD dwDataLen) {
  ...
  for (i = 0; i < dwDataLen / 2; i++) {
    ...
    if (nClock) {
      ...
      dcTemp.I =
          dcY.I * cos(PI * m_Clpf.dNCO_p) + dcY.Q * sin(PI * m_Clpf.dNCO_p);
      dcTemp.Q =
          dcY.Q * cos(PI * m_Clpf.dNCO_p) - dcY.I * sin(PI * m_Clpf.dNCO_p);
      ...
      if (m_Tlpf.nIndex == 1) {
        ...
        m_pDemodBuf[2 * dwCounter + 1] = LONG(
            SHRT_MAX * 0.125 * (fabs(m_Tlpf.dcMid.I + m_Tlpf.dcMid.Q) - 1.0));

        dwCounter++;

        ////载波误差////
        dPhase =
            4 * (atan2(m_Clpf.dcDelay[0].Q, m_Clpf.dcDelay[0].I) / PI + 0.0);
        while (dPhase > 1.0) {
          dPhase -= 2;
        }
        while (dPhase < -1.0) {
...
DWORD CSignalDemodProbe::Demod_BPSK(double *pData, DWORD dwDataLen) {
  ...
  for (i = 0; i < dwDataLen / 2; i++) {
    ...
    if (dAmplit > 4) {
      m_Agc.m = 4 / sqrt(pData[2 * i] * pData[2 * i] +
                         pData[2 * i + 1] * pData[2 * i + 1]);
      m_Agc.s = 20 * log10(m_Agc.m);
    }
  ...
  for (i = 0; i < dwDataLen / 2; i++) {
    ...
    if (nClock) {
      ...
      if (m_Tlpf.nIndex == 1) {
        ...
        ////环路滤波////
        m_Clpf.dLpf_cc += dPhase * m_Clpf.dLpf_c2 * 25;
        m_Clpf.dNCO_e = m_Clpf.dLpf_cc + dPhase * m_Clpf.dLpf_c1 * 1;
        ...
        ////环路滤波////
        m_Tlpf.dLpf_cc += dPhase * m_Tlpf.dLpf_c2;
        m_Tlpf.dDCO_e = m_Tlpf.dLpf_cc + dPhase * m_Tlpf.dLpf_c1 / 2;
  ...
  for (i = 0; i < dwDataLen / 2; i++) {
    ...
    if (nClock) {
      ...
      if (m_Tlpf.nIndex == 1) {
        ...
        ////环路滤波////
        m_Clpf.dLpf_cc += dPhase * m_Clpf.dLpf_c2 * 1.414f;
        m_Clpf.dNCO_e = m_Clpf.dLpf_cc + dPhase * m_Clpf.dLpf_c1 / 2.5f;
        ...
        ////环路滤波////
        m_Tlpf.dLpf_cc += dPhase * m_Tlpf.dLpf_c2 * 1;
        m_Tlpf.dDCO_e = m_Tlpf.dLpf_cc + dPhase * m_Tlpf.dLpf_c1 * 1 ;
...
DWORD CSignalDemodProbe::Demod_32QAM(double *pData, DWORD dwDataLen) {
  ...
  SHORT cntCA = 10;
  double CaliAmpl[] = {0.25 * 0.25 * 2,           0.75 * 0.75 * 2,
                       1.25 * 1.25 * 2,           1.75 * 1.75 * 2,
                       0.25 * 0.25 + 0.75 * 0.75, 0.25 * 0.25 + 1.25 * 1.25,
                       0.25 * 0.25 + 1.75 * 1.75, 0.75 * 0.75 + 1.25 * 1.25,
                       0.75 * 0.75 + 1.75 * 1.75, 1.25 * 1.25 + 1.75 * 1.75};
  ...
  for (i = 0; i < dwDataLen / 2; i++) {
    ...
    if (nClock) {
      ...
      if (m_Tlpf.nIndex == 1) {
        ...
        ////环路滤波////
        /* @xming: 15db可解调，但无法保证误码率 */
        m_Clpf.dLpf_cc += dPhase * m_Clpf.dLpf_c2 / (1.414 * 45);
        m_Clpf.dNCO_e = m_Clpf.dLpf_cc + dPhase * m_Clpf.dLpf_c1 / (10);
  ...
  for (i = 0; i < dwDataLen / 2; i++) {
    ...
    if (nClock) {
      ...
      if (m_Tlpf.nIndex == 1) {
        ...
        ////环路滤波////
        m_Clpf.dLpf_cc += dPhase * m_Clpf.dLpf_c2 / (1.414 * 15);
        m_Clpf.dNCO_e = m_Clpf.dLpf_cc + dPhase * m_Clpf.dLpf_c1 * 1.414;
        ...
        ////环路滤波////
        m_Tlpf.dLpf_cc += dPhase * m_Tlpf.dLpf_c2 / 1.414;
        m_Tlpf.dDCO_e = m_Tlpf.dLpf_cc + dPhase * m_Tlpf.dLpf_c1 / 1.414;
...
DWORD CSignalDemodProbe::Demod_128QAM(double *pData, DWORD dwDataLen) {
  DWORD i, dwCounter;
  INT k, nClock, nFIRIndex, M;
  ...
  SHORT cntCP = 14 - 1;
  double CaliPhase[] = {0.25f,                 // atan(1/1)/pi
                        0.102416382349567f,    // atan(1/3)/pi
                        0.0628329581890012f,   // atan(1/5)/pi
                        0.0451672353008665f,   // atan(1/7)/pi
                        0.035223287477277f,    // atan(1/9)/pi
                        0.028857938376304f,    // atan(1/11)/pi
                        0.172020869622631,     // atan(3/5)/pi
                        0.128881058409157,     // atan(3/7)/pi
                        0.084750659461432,     // atan(3/11)/pi
                        0.197431543288747,     // atan(5/7)/pi
                        0.161414467217095,     // atam(5/9)/pi
                        0.135799748780092,     // atan(5/11)/pi
                        0.210416575839434,     // atan(7/9)/pi
                        0.180395512726936      // atan(7/11)/pi
                        };
  SHORT cntCA = 10;
  double CaliAmpl[] = {0.25 * 0.25 * 2,           0.75 * 0.75 * 2,
                       1.25 * 1.25 * 2,           1.75 * 1.75 * 2,
  ...
  for (i = 0; i < dwDataLen / 2; i++) {
    ...
    if (nClock) {
      ...
      if (m_Tlpf.nIndex == 1) {
        ...
        ////环路滤波////
        m_Clpf.dLpf_cc += dPhase * m_Clpf.dLpf_c2 / (1.414 * 50);
        m_Clpf.dNCO_e = m_Clpf.dLpf_cc + dPhase * m_Clpf.dLpf_c1/10;
...
DWORD CSignalDemodProbe::Demod_256QAM(double *pData, DWORD dwDataLen) {
  DWORD i, dwCounter;
  INT k, nClock, nFIRIndex, M;
  ...
  SHORT cntCP = 7;
  double CaliPhase[] = {0.25,               /* atan() */
                        0.102416382349567,  /* atan() */
                        0.0628329581890012,
                        0.0451672353008665,
                        0.172020869622631,
                        0.128881058409157,
  ...
  for (i = 0; i < dwDataLen / 2; i++) {
    ...
    if (nClock) {
      ...
      if (m_Tlpf.nIndex == 1) {
        ...
        ////环路滤波////
        m_Clpf.dLpf_cc += dPhase * m_Clpf.dLpf_c2 / (1.414 * 50);
        m_Clpf.dNCO_e = m_Clpf.dLpf_cc + dPhase * m_Clpf.dLpf_c1/15;
...
DWORD CSignalDemodProbe::Decode_2Lvl(LONG *pData, DWORD dwDataLen) {
  ...
  memset(m_pBitBuf, 0, m_dwBufLen * sizeof(CHAR));
...
DWORD CSignalDemodProbe::Decode_4Lvl(LONG *pData, DWORD dwDataLen) {
  ...
  ////数据归一化///
  for (i = 0; i < dwDataLen / 2; i++) {
    m_pNormalizedDemodBuf[i] = pData[2 * i] * sqrt(5.0) / sqrt(dMeanValue);
  }

  // 格雷码逻辑映射 //
  for (i = 0; i < dwDataLen / 2; i++) {
    if ((m_pNormalizedDemodBuf[i] > 2)) // +3
    {
      m_pBitBuf[2 * i] = 0; // 1;
      m_pBitBuf[2 * i + 1] = 0;
    } else if (m_pNormalizedDemodBuf[i] > 0 && m_pNormalizedDemodBuf[i] < 2) // +1
    {
      m_pBitBuf[2 * i] = 0; // 1;
      m_pBitBuf[2 * i + 1] = 1;
    } else if (m_pNormalizedDemodBuf[i] < 0 && m_pNormalizedDemodBuf[i] > -2) // -1
    {
      m_pBitBuf[2 * i] = 1; // 0;
      m_pBitBuf[2 * i + 1] = 1;
    } else // if((m_pNormalizedDemodBuf[i] < -2))	// -3
    {
      m_pBitBuf[2 * i] = 1; // 0;
      m_pBitBuf[2 * i + 1] = 0;
    }
...
DWORD CSignalDemodProbe::Decode_4Phase(LONG *pData, DWORD dwDataLen) {
  ...
  if (m_bIsDecode) {
    ...
    ////求差分码////
    for (DWORD i = 1; i < dwDataLen / 2; i++) {
      byMod2Sum = (m_pBitBuf[2 * i] + m_pBitBuf[2 * i + 1] +
                   m_pBitBuf[2 * (i - 1)] + m_pBitBuf[2 * (i - 1) + 1]) %
                  2;
...
DWORD CSignalDemodProbe::Decode_8PSK(LONG *pData, DWORD dwDataLen) {
  ...
  // 格雷码逻辑映射
  for (DWORD i = 0; i < dwDataLen / 2; i++) {
    if (pData[2 * i] > 0 && pData[2 * i + 1] > 0 &&
        abs(pData[2 * i]) < abs(pData[2 * i + 1])) // 1
    {
      m_pBitBuf[3 * i] = 0;
      m_pBitBuf[3 * i + 1] = 0;
      m_pBitBuf[3 * i + 2] = 1;
    } else if (pData[2 * i] < 0 && pData[2 * i + 1] > 0 &&
               abs(pData[2 * i]) < abs(pData[2 * i + 1])) // 3
    {
      m_pBitBuf[3 * i] = 0;
      m_pBitBuf[3 * i + 1] = 1;
      m_pBitBuf[3 * i + 2] = 1;
    } else if (pData[2 * i] < 0 && pData[2 * i + 1] > 0 &&
               abs(pData[2 * i]) > abs(pData[2 * i + 1])) // 2
    {
      m_pBitBuf[3 * i] = 0;
      m_pBitBuf[3 * i + 1] = 1;
      m_pBitBuf[3 * i + 2] = 0;
    } else if (pData[2 * i] < 0 && pData[2 * i + 1] < 0 &&
               abs(pData[2 * i]) > abs(pData[2 * i + 1])) // 6
    {
      m_pBitBuf[3 * i] = 1;
      m_pBitBuf[3 * i + 1] = 1;
      m_pBitBuf[3 * i + 2] = 0;
    } else if (pData[2 * i] < 0 && pData[2 * i + 1] < 0 &&
               abs(pData[2 * i]) < abs(pData[2 * i + 1])) // 7
    {
      m_pBitBuf[3 * i] = 1;
      m_pBitBuf[3 * i + 1] = 1;
      m_pBitBuf[3 * i + 2] = 1;
    } else if (pData[2 * i] > 0 && pData[2 * i + 1] < 0 &&
               abs(pData[2 * i]) < abs(pData[2 * i + 1])) // 5
    {
      m_pBitBuf[3 * i] = 1;
      m_pBitBuf[3 * i + 1] = 0;
      m_pBitBuf[3 * i + 2] = 1;
    } else if (pData[2 * i] > 0 && pData[2 * i + 1] < 0 &&
               abs(pData[2 * i]) > abs(pData[2 * i + 1])) // 4
    {
      m_pBitBuf[3 * i] = 1;
      m_pBitBuf[3 * i + 1] = 0;
      m_pBitBuf[3 * i + 2] = 0;
    } else //( pData[2*i]>0 && pData[2*i+1]>0 &&
           // abs(pData[2*i])>abs(pData[2*i+1]) )		//0
    {
      m_pBitBuf[3 * i] = 0;
      m_pBitBuf[3 * i + 1] = 0;
      m_pBitBuf[3 * i + 2] = 0;
  ...
  if (m_bIsDecode) {
    ...
    for (DWORD i = 1; i < dwDataLen / 2; i++) {
      byMod2Sum0 =
          (m_pBitBuf[3 * i - 3] + m_pBitBuf[3 * i - 2] + m_pBitBuf[3 * i - 1]) %
          2;
      ...
      if (byMod2Sum0 == 0) {
        ...
        if (byMod2Sum1 == 0) {
          if (byMod2Sum2 == 0) {
            byCode0 = (m_pBitBuf[3 * i] + m_pBitBuf[3 * i - 3]) % 2;
            byCode1 =
                (byCode0 + m_pBitBuf[3 * i + 2] + m_pBitBuf[3 * i - 1]) % 2;
            byCode2 =
                (byCode0 + m_pBitBuf[3 * i + 1] + m_pBitBuf[3 * i - 2]) % 2;
          } else {
            byCode1 = (m_pBitBuf[3 * i + 1] + m_pBitBuf[3 * i - 2]) % 2;
            byCode0 =
                (byCode1 + m_pBitBuf[3 * i + 2] + m_pBitBuf[3 * i - 1]) % 2;
...
DWORD CSignalDemodProbe::Decode_16QAM(LONG *pData, DWORD dwDataLen) {
  ...
  if (m_pNormalizedDemodBuf != NULL && m_dwSgnlLen < dwDataLen) {
    delete[] m_pNormalizedDemodBuf;
  ...
  ////数据归一化///
  for (i = 0; i < dwDataLen / 2; i++) {
    m_pNormalizedDemodBuf[2 * i] = pData[2 * i] * sqrt(5.0) / sqrt(dMeanValueI);
    m_pNormalizedDemodBuf[2 * i + 1] = pData[2 * i + 1] * sqrt(5.0) / sqrt(dMeanValueQ);
  }

  // 格雷码逻辑映射 //
  for (i = 0; i < dwDataLen / 2; i++) {
    if (m_pNormalizedDemodBuf[2 * i] > 0 && m_pNormalizedDemodBuf[2 * i + 1] > 0) // 第一象限
    {
      m_pBitBuf[4 * i] = 0;
      m_pBitBuf[4 * i + 1] = 0;
    } else if (m_pNormalizedDemodBuf[2 * i] < 0 && m_pNormalizedDemodBuf[2 * i + 1] > 0) // 第二象限
    {
      m_pBitBuf[4 * i] = 0;
      m_pBitBuf[4 * i + 1] = 1;
    } else if (m_pNormalizedDemodBuf[2 * i] < 0 && m_pNormalizedDemodBuf[2 * i + 1] < 0) // 第三象限
    {
      m_pBitBuf[4 * i] = 1;
      m_pBitBuf[4 * i + 1] = 1;
    } else // if(m_pNormalizedDemodBuf[2*i]>0 && m_pNormalizedDemodBuf[2*i+1]<0)	//第四象限
    {
      m_pBitBuf[4 * i] = 1;
      m_pBitBuf[4 * i + 1] = 0;
    }

    if (fabs(m_pNormalizedDemodBuf[2 * i]) > 2 && fabs(m_pNormalizedDemodBuf[2 * i + 1]) > 2) // 1
    {
      m_pBitBuf[4 * i + 2] = 1;
      m_pBitBuf[4 * i + 3] = 1;
    } else if (fabs(m_pNormalizedDemodBuf[2 * i]) < 2 && fabs(m_pNormalizedDemodBuf[2 * i + 1]) > 2 &&
               m_pNormalizedDemodBuf[2 * i] * m_pNormalizedDemodBuf[2 * i + 1] > 0) //[1,3],[-1,-3]
    {
      m_pBitBuf[4 * i + 2] = 1;
      m_pBitBuf[4 * i + 3] = 0;
    } else if (fabs(m_pNormalizedDemodBuf[2 * i]) > 2 && fabs(m_pNormalizedDemodBuf[2 * i + 1]) < 2 &&
               m_pNormalizedDemodBuf[2 * i] * m_pNormalizedDemodBuf[2 * i + 1] < 0) //[3,-1],[-3,1]
    {
      m_pBitBuf[4 * i + 2] = 1;
      m_pBitBuf[4 * i + 3] = 0;
    } else if (fabs(m_pNormalizedDemodBuf[2 * i]) < 2 &&
               fabs(m_pNormalizedDemodBuf[2 * i + 1]) < 2) // 3
    {
      m_pBitBuf[4 * i + 2] = 0;
      m_pBitBuf[4 * i + 3] = 0;
    } else if (fabs(m_pNormalizedDemodBuf[2 * i]) < 2 && fabs(m_pNormalizedDemodBuf[2 * i + 1]) > 2 &&
  ...
  if (m_bIsDecode) {
    ...
    ////求差分码////
    for (i = 1; i < dwDataLen / 2; i++) {
      byMod2Sum = (m_pBitBuf[4 * i] + m_pBitBuf[4 * i + 1] +
                   m_pBitBuf[4 * (i - 1)] + m_pBitBuf[4 * (i - 1) + 1]) %
                  2;
...
DWORD CSignalDemodProbe::Decode_32QAM(LONG *pData, DWORD dwDataLen) {
  ...
  ////数据归一化///
  for (i = 0; i < dwDataLen / 2; i++) {
    m_pNormalizedDemodBuf[2 * i] = pData[2 * i] * sqrt(35.0 / 3) / sqrt(dMeanValueI);
    m_pNormalizedDemodBuf[2 * i + 1] =
        pData[2 * i + 1] * sqrt(35.0 / 3) / sqrt(dMeanValueQ);
  }
  ...
  for (i = 0; i < dwDataLen / 2; i++) {
    ...
    if (fabs(m_pNormalizedDemodBuf[2 * i]) < 2 && fabs(m_pNormalizedDemodBuf[2 * i + 1]) < 2) // 1
    {
      ...
      m_pBitBuf[5 * i + 4] = 1;
               ...
               fabs(m_pNormalizedDemodBuf[2 * i + 1]) > 2 && // 5
               fabs(m_pNormalizedDemodBuf[2 * i + 1]) < 4) {
      m_pBitBuf[5 * i + 2] = 1;
      m_pBitBuf[5 * i + 3] = 0;
      m_pBitBuf[5 * i + 4] = 1;
    } else if (fabs(m_pNormalizedDemodBuf[2 * i]) < 2 &&
               fabs(m_pNormalizedDemodBuf[2 * i + 1]) > 4) // 4
    {
      m_pBitBuf[5 * i + 2] = 1;
      m_pBitBuf[5 * i + 3] = 0;
      m_pBitBuf[5 * i + 4] = 0;
    } else if (fabs(m_pNormalizedDemodBuf[2 * i]) > 2 && fabs(m_pNormalizedDemodBuf[2 * i]) < 4 &&
               fabs(m_pNormalizedDemodBuf[2 * i + 1]) < 2) // 2
    {
      m_pBitBuf[5 * i + 2] = 0;
      m_pBitBuf[5 * i + 3] = 1;
      m_pBitBuf[5 * i + 4] = 0;
    } else if (fabs(m_pNormalizedDemodBuf[2 * i]) > 2 && fabs(m_pNormalizedDemodBuf[2 * i]) < 4 &&
               fabs(m_pNormalizedDemodBuf[2 * i + 1]) > 2 &&
               fabs(m_pNormalizedDemodBuf[2 * i + 1]) < 4) // 6
    {
      m_pBitBuf[5 * i + 2] = 1;
      m_pBitBuf[5 * i + 3] = 1;
      m_pBitBuf[5 * i + 4] = 0;
    } else if (fabs(m_pNormalizedDemodBuf[2 * i]) > 2 && fabs(m_pNormalizedDemodBuf[2 * i]) < 4 &&
               fabs(m_pNormalizedDemodBuf[2 * i + 1]) > 4) // 0
    {
      m_pBitBuf[5 * i + 2] = 0;
      m_pBitBuf[5 * i + 3] = 0;
      m_pBitBuf[5 * i + 4] = 0;
    } else if (fabs(m_pNormalizedDemodBuf[2 * i]) > 4 &&
               fabs(m_pNormalizedDemodBuf[2 * i + 1]) < 2) // 3
    {
      m_pBitBuf[5 * i + 2] = 0;
      m_pBitBuf[5 * i + 3] = 1;
      m_pBitBuf[5 * i + 4] = 1;
    } else // if(  fabs(m_pNormalizedDemodBuf[2*i]) > 4 && fabs(m_pNormalizedDemodBuf[2*i+1]) > 2  &&
           // fabs(m_pNormalizedDemodBuf[2*i+1]) < 4)	//7
    {
      m_pBitBuf[5 * i + 2] = 1;
      m_pBitBuf[5 * i + 3] = 1;
      m_pBitBuf[5 * i + 4] = 1;
  ...
  if (m_bIsDecode) {
    ...
    ////求差分码////
    for (i = 1; i < dwDataLen / 2; i++) {
      byMod2Sum = (m_pBitBuf[5 * i] + m_pBitBuf[5 * i + 1] +
                   m_pBitBuf[5 * (i - 1)] + m_pBitBuf[5 * (i - 1) + 1]) %
                  2;
...
DWORD CSignalDemodProbe::Decode_64QAM(LONG *pData, DWORD dwDataLen) {
  ...
  ////数据归一化///
  for (i = 0; i < dwDataLen / 2; i++) {
    m_pNormalizedDemodBuf[2 * i] = pData[2 * i] * sqrt(21.0) / sqrt(dMeanValueI);
    m_pNormalizedDemodBuf[2 * i + 1] = pData[2 * i + 1] * sqrt(21.0) / sqrt(dMeanValueQ);
  }
  ...
  for (i = 0; i < dwDataLen / 2; i++) {
    ...
    if (m_pNormalizedDemodBuf[2 * i] - 4 > 0 && m_pNormalizedDemodBuf[2 * i + 1] - 4 > 0) //
    {
      m_pBitBuf[6 * i + 2] = 1;
      m_pBitBuf[6 * i + 3] = 1;
    } else if (m_pNormalizedDemodBuf[2 * i] - 4 < 0 && m_pNormalizedDemodBuf[2 * i + 1] - 4 > 0) //
    {
      m_pBitBuf[6 * i + 2] = 1;
      m_pBitBuf[6 * i + 3] = 0;
    } else if (m_pNormalizedDemodBuf[2 * i] - 4 < 0 && m_pNormalizedDemodBuf[2 * i + 1] - 4 < 0) //
    ...
    if (fabs(m_pNormalizedDemodBuf[2 * i] - 4) < 2 &&
        fabs(m_pNormalizedDemodBuf[2 * i + 1] - 4) < 2) //
    {
      m_pBitBuf[6 * i + 4] = 1;
      m_pBitBuf[6 * i + 5] = 1;
    } else if (fabs(m_pNormalizedDemodBuf[2 * i] - 4) > 2 &&
               fabs(m_pNormalizedDemodBuf[2 * i + 1] - 4) < 2) //
    {
      m_pBitBuf[6 * i + 4] = 1;
      m_pBitBuf[6 * i + 5] = 0;
               ...
               fabs(m_pNormalizedDemodBuf[2 * i + 1] - 4) > 2) //
    {
      m_pBitBuf[6 * i + 4] = 0;
      m_pBitBuf[6 * i + 5] = 0;
    } else // if( fabs(m_pNormalizedDemodBuf[2*i] - 4) < 2 && fabs(m_pNormalizedDemodBuf[2*i+1] - 4) >
           // 2)	//
    {
      m_pBitBuf[6 * i + 4] = 0;
      m_pBitBuf[6 * i + 5] = 1;
  ...
  if (m_bIsDecode) {
    ...
    ////求差分码////
    for (i = 1; i < dwDataLen / 2; i++) {
      byMod2Sum = (m_pBitBuf[6 * i] + m_pBitBuf[6 * i + 1] +
                   m_pBitBuf[6 * (i - 1)] + m_pBitBuf[6 * (i - 1) + 1]) %
                  2;
...
DWORD CSignalDemodProbe::Decode_128QAM(LONG *pData, DWORD dwDataLen) {
  DWORD i = 0;
  ...
  if (m_pBitBuf == NULL) {
    m_dwBufLen = 4 * dwDataLen;

    m_pBitBuf = new CHAR[m_dwBufLen];
  }

  m_dwBitNum = 4 * dwDataLen;
  memset(m_pBitBuf, 0, m_dwBufLen * sizeof(CHAR));
  ...
  ////数据归一化///
  for (i = 0; i < dwDataLen / 2; i++) {
    m_pNormalizedDemodBuf[2 * i] = pData[2 * i] * sqrt(85.0) / sqrt(dMeanValueI);
    m_pNormalizedDemodBuf[2 * i + 1] = pData[2 * i + 1] * sqrt(85.0) / sqrt(dMeanValueQ);
  }

  /*double m_pSgnlBuf2[dwDataLen];
  for (i=0;i<dwDataLen;i++) {
      m_pSgnlBuf2[i] = m_pNormalizedDemodBuf[i];
  } */
  ...
  for (i = 0; i < dwDataLen / 2; i++) {
    ...
    // bit2 and bit3
    if (m_pNormalizedDemodBuf[2 * i] - 6 > 0 && m_pNormalizedDemodBuf[2 * i + 1] - 6 > 0) //
    {
      m_pBitBuf[8 * i + 2] = 1;
      m_pBitBuf[8 * i + 3] = 1;
    } else if (m_pNormalizedDemodBuf[2 * i] - 6 < 0 && m_pNormalizedDemodBuf[2 * i + 1] - 6 > 0) //
    {
      m_pBitBuf[8 * i + 2] = 1;
      m_pBitBuf[8 * i + 3] = 0;
    } else if (m_pNormalizedDemodBuf[2 * i] - 6 < 0 && m_pNormalizedDemodBuf[2 * i + 1] - 6 < 0) //
    ...
    // bit6 and bit7
    if (m_pNormalizedDemodBuf[2 * i] - 2 > 0 && m_pNormalizedDemodBuf[2 * i + 1] - 2 > 0) //
    {
      m_pBitBuf[8 * i + 6] = 1;
      m_pBitBuf[8 * i + 7] = 1;
    } else if (m_pNormalizedDemodBuf[2 * i] - 2 < 0 && m_pNormalizedDemodBuf[2 * i + 1] - 2 > 0) //
    {
      m_pBitBuf[8 * i + 6] = 1;
      m_pBitBuf[8 * i + 7] = 0;
    } else if (m_pNormalizedDemodBuf[2 * i] - 2 < 0 && m_pNormalizedDemodBuf[2 * i + 1] - 2 < 0) //
  ...
  if (m_bIsDecode) {
    ...
    ////求差分码////
    for (i = 1; i < dwDataLen / 2; i++) {
      byMod2Sum = (m_pBitBuf[8 * i] + m_pBitBuf[8 * i + 1] +
                   m_pBitBuf[8 * (i - 1)] + m_pBitBuf[8 * (i - 1) + 1]) %
                  2;

      if (byMod2Sum == 0) {
        byCode0 = (m_pBitBuf[8 * i] + m_pBitBuf[8 * (i - 1)]) % 2;
        byCode1 = (m_pBitBuf[8 * i + 1] + m_pBitBuf[8 * (i - 1) + 1]) % 2;
      } else {
        byCode0 = (m_pBitBuf[8 * i] + m_pBitBuf[8 * (i - 1) + 1]) % 2;
        byCode1 = (m_pBitBuf[8 * i + 1] + m_pBitBuf[8 * (i - 1)]) % 2;
      }

      m_pBitBuf[8 * i] = byCode0;
      m_pBitBuf[8 * i + 1] = byCode1;
    }

    ////保存本次输入最后一个点的检测码////
    m_PreCode[0] = m_pBitBuf[m_dwBitNum - 6];
    m_PreCode[1] = m_pBitBuf[m_dwBitNum - 5];
  }
...
DWORD CSignalDemodProbe::Decode_256QAM(LONG *pData, DWORD dwDataLen) {
  DWORD i = 0;
...
DWORD CSignalDemodProbe::Decode_16APSK( LONG *pData,DWORD dwDataLen )
{
    ...
    for (i = 0; i < dwDataLen/2; i++)
    {
        ...
        else if( fabs(m_pNormalizedDemodBuf[2*i])<2 && fabs(m_pNormalizedDemodBuf[2*i+1]) > 2 )  //[1,3],[-1,-3]
        {
            m_pBitBuf[4*i+2]=1;
            m_pBitBuf[4*i+3]=0;
        }
        else if( fabs(m_pNormalizedDemodBuf[2*i]) > 2 && fabs(m_pNormalizedDemodBuf[2*i+1]) < 2 )	//[3,-1],[-3,1]
        {
            m_pBitBuf[4*i+2]=0;
            m_pBitBuf[4*i+3]=1;
        }
