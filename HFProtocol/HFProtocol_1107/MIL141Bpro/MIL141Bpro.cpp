...
void CMIL141Bpro::MIL141Bdemode_ini(int nLeng, int Insample, int Outsample, Ipp16s P, Ipp32f roll, Ipp32f Baud, int SrctapLen)
{
    waveType = 0;
    const int allLen = 3 * nLeng;
...
void CMIL141Bpro::MIL141Bdemode(Ipp16s* pSrc, int nLeng, Ipp16s P, Ipp16s* expout, int& outLeng, Ipp8u* outbyte, int& byteLeng, HeadType141B* headType, int& headNum)
{
    ...
    else
    {
        ...
        headNum = 0;
        if (m_BW0demode != NULL) {
            m_BW0demode->ResetBurstCandidates();
        }
    }
...
void CMIL141Bpro::IppReSample_16s(Ipp16s* pSrc, int nLeng, Ipp16s* pDst, int& outLen)
{
    ...
    lastread += nLeng;
    ippsResamplePolyphaseFixed_16s(state, inBuf, lastread - nHistory - (int)time, pDst, 0.98f, &time, &outLen);
...
int CMIL141Bpro::GetBW0BurstCandidateCount() const
{
    return m_BW0demode == NULL ? 0 : m_BW0demode->GetBurstCandidateCount();
}

const BW0BurstCandidate* CMIL141Bpro::GetBW0BurstCandidates() const
{
    return m_BW0demode == NULL ? NULL : m_BW0demode->GetBurstCandidates();
}

void CMIL141Bpro::ClearBW0BurstCandidates()
{
    if (m_BW0demode != NULL) {
        m_BW0demode->ResetBurstCandidates();
    }
