...
void CPreambledetect::Preamble_Gen(int type)
{
	...
	if(type==3)
	{
		...
		PSK_MOD(TCLAGC_141B,PreambleLen,8,pPreambleSym);
	}
	else if(type==4)
	{
		PreambleLen = 384;
		if(pPreambleSym!=NULL)
			ippsFree(pPreambleSym);
		pPreambleSym = ippsMalloc_32fc(PreambleLen);
		PSK_MOD(preamble_BW0,PreambleLen,8,pPreambleSym);
	}
	else if(type==5)
	{
		PreambleLen = 512;
		if(pPreambleSym!=NULL)
			ippsFree(pPreambleSym);
		pPreambleSym = ippsMalloc_32fc(PreambleLen);
		PSK_MOD(preamble_BW1,PreambleLen,8,pPreambleSym);
	}
	else if(type==6)
	{
		PreambleLen = 512;
		if(pPreambleSym!=NULL)
			ippsFree(pPreambleSym);
		pPreambleSym = ippsMalloc_32fc(PreambleLen);
		PSK_MOD(preamble_BW3,PreambleLen,8,pPreambleSym);
	}
}
