...
int CDataFIRDF::IinitialLPFPara(short nLPFDesiType,double dPassWidth,double dStopWidth,
...
// 获取成员变量 m_nLPFLen
short CDataFIRDF::get_filter_len(void) {
	return m_nLPFLen;
}

// 设置成员变量
short CDataFIRDF::set_filter_len(short len) {
	if (len <= 0) {
		return ERROR_FILTER_LEN;
	}
	m_nLPFLen = len;
	return m_nLPFLen;
}
...
// 获取滤波器系数
double* CDataFIRDF::get_filter_para(void) {
	// 这里不应该直接返回滤波器系数的指针
	return m_pLPFBuf;
}
