...
class  AFX_EXT_CLASS CMIL110Bpro
{
public:
	CMIL110Bpro(void);
	~CMIL110Bpro(void);
...
public:
	...
	void IdentifyType(Ipp32fc *pSrc,int nLeng,Ipp32fc **Typesym,int typeNum,int &outtype);
	...
	void Synchronization_QAM(Ipp32fc *FrameBuf,Ipp16s FrameLen,Ipp16s P,Ipp32fc *pUWsig,Ipp16s UWleng,int delayCma,
	...
	void EqualizationRLS_DFE_PLL(Ipp32fc *pSrcDst,int nLeng,int P,int delay,Ipp32f mEnergy,int M,Ipp32fc *UWsig,Ipp32fc *pDst,int &outLen,BOOL aid);


	void DecPSK(Ipp32fc pSrc,Ipp32fc &pDst);
	void DecQAM(Ipp32fc pSrc,int M,Ipp32fc &pDst,Ipp8u &num);
	void judgeQAM110B(Ipp32fc *pSrc,int nLeng,int M,Ipp8u *pDst,int &outLeng);
	...
	void judgePSK110B(Ipp32fc *pSrc,int nLeng,Ipp8u *pDst,int &outLeng,Ipp8s modutype);

	void DeBitPair_PhaseDecode(Ipp8u *pSrc,int nLeng,Ipp8u *pDst,int &outLeng);
	void deinterleaver(Ipp8u *pSrc,Ipp8u *pDst);

	void DeHead110B(Ipp16s *BurstPos,Ipp16s *TypePos,int BurstNum,HeadType110B *headType,int &headNum)
;
};
