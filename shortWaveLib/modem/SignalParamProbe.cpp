...
CSignalParamProbe::CSignalParamProbe(void)
{
    ...
    m_sap.nPsd12THPeakNums = 0;
    m_sap.nPsd12ThPeakIdx = 0;
										...
										SHORT nModuType /*= SMT_UNKNOWN*/)
{
    ...
    if ((nModuType < SMT_UNKNOWN) || (nModuType > SMT_16APSK))
	{
	...
	memset(m_pAmplPSD,0,m_dwFFTSize*sizeof(double));
...
//    printf("m_dwSignLen = %d ",m_dwSignLen);
	...
	memset(m_pDataBuff,0,m_dwSignLen*sizeof(double));
	memset(m_pSignalBuff,0,m_dwSignLen*sizeof(double));
	memset(m_pAmplBuff,0,m_dwSignLen*sizeof(double)/2);
	memset(m_pFreqBuff,0,m_dwSignLen*sizeof(double)/2);
	...
	memset(m_pTmpBufI,0,m_dwTmpBufLen*sizeof(double));
	memset(m_pTmpBufQ,0,m_dwTmpBufLen*sizeof(double));	
...
DWORD CSignalParamProbe::InputData(double *pData, DWORD dwDataLen)
{
	...
	memcpy(m_pDataBuff + m_dwIQLen,pData,dwIQLen*sizeof(double));
...
DWORD CSignalParamProbe::GetSignalParam(SIGNAL_DEMOD_PARAM& pSignalPara)
{
	...
	/// ?????í??????????·?°ü???????????÷???ù????·??????? ///
	auto search = umap_input_uint8.find(nModuType);
	if (search != umap_input_uint8.end()) {
		...
		pSignalPara.dEstimSNR = m_dSnrEsti;
	}
	// for (SHORT i = 0; i < SMT_NUM_MAX; i++)
	// {
...
DWORD CSignalParamProbe::ParamEstimate( double *pData, DWORD dwDataLen )
{
    ...
    switch(m_nModuType) {
        ...
        default:
            {
                memcpy(m_pSignalBuff,pData,dwDataLen*sizeof(double));
...
SHORT CSignalParamProbe::ModuRecognition(double *pData, DWORD dwDataLen)
{
	...
	//////////////////////////////////////////////////////////////////////////
	// ?????×???÷??????±? MSK??BPSK??QPSK??8PSK??OQPSK??PI/4DQPSK??16QAM
	...
	if(dBandwidth>0.005 && dBandwidth<0.3 && m_sap.dAmplAveSqu<0.5)
	{		
		/* BPSK的平方谱有1个峰值 */
		if ((m_sap.nSpecType == 0) && (m_sap.nSpecPeakNum == 1) && m_sap.dFMForOrder > 2.5 &&
			(m_sap.dSpecPeakLvl > 300.0001))	
		{
			nModuType = SMT_BPSK;
		}
  ...
  //      if ((m_sap.nSpecType == 1) &&  (m_sap.dAmplAveSqu < 0.08) && (m_sap.dFMForOrder < 2.0) &&
  //          (m_sap.dFMForOrder > 1.5) && (m_sap.dAMForOrder > 1.5)) //&& m_sap.dFMStatWight > 0.5 && (m_sap.nSpecPeakNum == 1)
		//{
		//	nModuType = SMT_GMSK;
		//}	
		...
		/* 	1. 与QPSK相比，16QAM的 dSpecPeakLvl 会低于QPSK，在25db的情况下都不会高于85，但高于50
			2. 与QPSK相比，16QAM的 AmstatWight 会比较高，分布在25~30之间。而 QPSK 的 AMStatWight 小于20。
			3. 16QAM 的
		*/
        //if ((m_sap.nSpecType == 1) && (m_sap.nSpecPeakNum == 1) && (m_sap.dDASPeakLvl > 0.05) &&
        //    (m_sap.dAMStatWight > 0.250001) && m_sap.dFMForOrder > 2.5 && m_sap.dAmplAveSqu > 0.35)
        //{
        //    nModuType = SMT_16QAM;
        //    if (m_sap.dDASPeakLvl < 0.16) {
        //        m_nModuType = SMT_64QAM;
        //    }

        //    if (m_sap.dFMForOrder < 5.0) {
        //        m_nModuType = SMT_32QAM;
        //    }
        //}

        //// 16APSK 的十二次方谱有 1 根谱线
        //if ((m_sap.nPsd12THPeakNums == 1) && (m_sap.nSpecType == 0 && m_sap.nSpecPeakNum != 1) && (m_sap.dAmplAveSqu > 0.385) &&
        //    (m_sap.dAMStatWight > 0.32) )
        //{
        //    nModuType = SMT_16APSK;
        //}



		/* 	64QAM与16QAM十分相似，但 64QAM 的 specPeakLvl 在区间 [35,45)中，而 16QAM 都保持在50+
	       	@warning specPeakLvl 是功率谱谱峰相对电平，与符号速率、采样率息息相关。下面的判决条件在
			fs=5Rb的情况下是准确的。
		*/
//        if ((m_sap.nSpecType == 1) && (m_sap.nSpecPeakNum == 1) && (m_sap.dDASPeakLvl > 0.05) &&
//            (m_sap.dAMStatWight > 0.220001) && m_sap.dFMForOrder > 2.5 &&
//            (m_sap.dSpecPeakLvl > 30.0001 && m_sap.dSpecPeakLvl < 45.0001))
//        {
//            nModuType = SMT_64QAM;
//        }


        /*	1. 32QAM SpecType=1, 且 SpecPeakNum = 2, 在高信噪比下，SpecPeakNum = 1
            2. 32QAM 的 AmstatWight 与 16QAM 相似
            3. 32QAM 的 dPsdSymmet 为负数
            4. 32QAM 的 dSpecPeakLvl，当 SNR 在区间 10db-25db 时，分布在 9.7-11 中
        */
//        if ((m_sap.nSpecType == 1) && (m_sap.nSpecPeakNum == 1 || m_sap.nSpecPeakNum == 2) &&
//            (m_sap.dDASPeakLvl > 0.10) && m_sap.dFMForOrder > 2.2 &&
//            (m_sap.dSpecPeakLvl > 15.0001))
//        {
//            nModuType = SMT_32QAM;
//        }
        ...
        case SMT_16APSK:
        {
            dTempValue = m_sap.nPsd12ThPeakIdx - 0.5 * m_dwFFTSize;
            m_dSignalFreq = dTempValue / 2 / pow(2.0, m_sap.nSpecType + 1.0);
        }
		default:
			{
				/*  BPSK/QPSK/8PSK/16QAM/PI4DQPSK/SDPSK/MQAM/MAPSK差分幅度谱的峰值位置 */
				m_dSymbolRate = m_sap.nDASPeakIdx;
			}
			break;
		}	
	...
	if (nModuType == SMT_UNKNOWN)
	{
		...
		/* 单音FM调制 */
		if( (m_sap.dDFSPeakLvl - m_sap.dDASPeakLvl) > -10 && (m_sap.dFMForOrder < 2.5) && 
		...
		/* 音频/电台FM调制 */
        if( m_sap.dFMForOrder < 10.0 && m_sap.dAMForOrder > 2.0  && m_sap.dAmplAveSqu < 0.15 &&
		...
		/* 音频/电台AM调制 */
        if( m_sap.dFMForOrder > 2.5 && m_sap.dAMForOrder < 8.0 && m_sap.dAmplAveSqu > 0.05 &&
            m_sap.dFreqAveSqu < 0.98 && m_sap.dDFSPeakLvl < 0.15)
		{
			nModuType = SMT_AM;
        }

        /* 信号源CW/SSB/AM/FM信号 */
        if ((m_sap.nPsdPeakNum == 1) && m_sap.dFMStatWight < 0.2 &&
             m_sap.dAmplAveSqu < 0.08)
        {
            nModuType = SMT_CW;
        }
		...
		// 0.5 =< FD/SR <= 4 //
		if( (m_sap.dDFSPeakLvl - m_sap.dDASPeakLvl) < 0.2 && (m_sap.dFMForOrder < 1.8) && (m_sap.dAMForOrder > 2.5) && 
			(m_sap.dFreqAveSqu > 0.2) && (m_sap.dAmplAveSqu < 0.25) && m_sap.dFMStatWight > 0.6) // && m_sap.dFMStatWight > 0.6
		{
			nModuType = SMT_2FSK;
		}		

        //if(	(m_sap.dDFSPeakLvl - m_sap.dDASPeakLvl) < 0.2 && (m_sap.dFMForOrder > 1.7) && (m_sap.dFMForOrder < 2.5) &&
        //    (m_sap.dAMForOrder > 2) && (m_sap.dFreqAveSqu > 0.5) && (m_sap.dAmplAveSqu < 0.2) && (m_sap.nPsdPeakNum == 4))// && m_sap.dFMStatWight > 0.35 && m_sap.dFMStatWight < 0.65
        //{
        //    nModuType = SMT_4FSK;
        //}
  ...
  //      if( m_sap.nPsdPeakNum == 1 && m_sap.dAMStatWight > 0.3 && m_sap.dAMForOrder > 1.5 && m_sap.dFMForOrder > 3.5 &&
...
DWORD CSignalParamProbe::GetSpecAnalParam(double *pData, DWORD dwDataLen)
{
    ...
    double* signal4th = (double *)malloc(sizeof(double) * dwDataLen);
    memmove(signal4th, pData, sizeof(double) * dwDataLen);
    ...
    // 信号十二次方谱分析
    double* signal8th = (double*)malloc(sizeof(double) * dwDataLen);
    memmove(signal8th, pData, sizeof(double) * dwDataLen);

    for (int i = 0; i < dwDataLen/2; i++) {
        double a = signal4th[2 * i];
        double b = signal4th[2 * i + 1];
        double c = signal8th[2 * i];
        double d = signal8th[2 * i + 1];
        // (x .^ 4) .* (x .^ 8)
        pData[2 * i] = a * c - b * d;
        pData[2 * i + 1] = a * d + b * c;
    }
    CalAverPowerSpecrum(m_pSignPsd, pData, dwDataLen);
    GetSpectraParam(pSpecParam12, m_pSignPsd, m_dwFFTSize);
    ...
    m_sap.nPsd12THPeakNums = (DWORD)pSpecParam12[0];
    m_sap.nPsd12ThPeakIdx = (DWORD)pSpecParam12[1];
...
DWORD CSignalParamProbe::CalAverPowerSpecrum(double* pPoweSpec,double *pData, DWORD dwDataLen)
{
	...
	memset(pPoweSpec,0,m_dwFFTSize*sizeof(double));
	...
	if (pSpecBuf != NULL)
	{
		memcpy(pPoweSpec,pSpecBuf,m_dwFFTSize*sizeof(double));
...
void CSignalParamProbe::CalAmplAverPowerSpecrum(double* pPoweSpec,double *pData, DWORD dwDataLen)
{
	...
	if (pSpecBuf != NULL)
	{
		memcpy(pPoweSpec,pSpecBuf,dwSpecLen*sizeof(double));
...
double CSignalParamProbe::CalASKSymbolRate( double *pDataIQ,DWORD dwDataLen )
{
	...
	if (pSpecBuf != NULL)
	{
		memcpy(m_pSignPsd,pSpecBuf,dwSpecLen*sizeof(double));
...
double* CSignalParamProbe::snrEstimation(double* pdataIQ, DWORD dwDatalen) {
	DWORD specLen = 0;
	m_pFFTIQ->InputData(pdataIQ, dwDatalen);
	double* specBuf = m_pFFTIQ->GetSpectrum(specLen);

	int freq_size = m_dSignalFreq / m_dSamplingFreq * m_dwFFTSize;
	// freq_size = freq_size > 0 ? ((int)freq_size + 1) : ((int)freq_size - 1);
	int sign_bw_size = ((int)(m_dSymbolRate * 1.35 / m_dSamplingFreq * m_dwFFTSize));

	
	int signal_start_index = m_dwFFTSize / 2 + freq_size - (sign_bw_size / 2);	// 信号起始下标
	int signal_end_index = m_dwFFTSize / 2 + freq_size + (sign_bw_size / 2);	// 信号结束小标
	int signal_range_index = signal_end_index - signal_start_index;

    if (signal_start_index < 0 || signal_end_index < 0) {
        m_dSnrEsti = 0;
        return specBuf;
    }
	...
	for (int i = signal_start_index; i < signal_end_index; i ++) {
		temp = specBuf[i] * specBuf[i];
		signal_power += temp / (signal_range_index * 1.0f);
	}

	/* 计算噪声功率 */
	double noise_power = 0;
	int noise_start_index = 100.0;
	int noise_end_index = signal_start_index - 100;
	int noise_range_index = noise_end_index - noise_start_index;
	for (int i = noise_start_index; i < noise_end_index - 100; i++) {
		temp = specBuf[i] * specBuf[i];
		noise_power += temp / (noise_range_index * 1.0f);
	}

	m_dSnrEsti = 10.0f * log10((signal_power - noise_power) / noise_power);
