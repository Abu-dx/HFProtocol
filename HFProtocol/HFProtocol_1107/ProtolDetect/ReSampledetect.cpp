...
CReSampledetect::CReSampledetect(void)
{
}
...
void CReSampledetect::ReSample_ini(Ipp32s nLeng,Ipp32f insample,Ipp32f outsample,Ipp16s incimate,Ipp16s tapsLen)
{
	m_Insample = insample;
...
void CReSampledetect::ReSample_Free()
{
...
void  CReSampledetect::ReSample(Ipp16s *pSrc,int nLeng,Ipp32fc *pDst,int &outLeng)
{
	int i,m=0;
	Ipp32fc temp;
	...
	if(m_Insample == m_Outsample)
	{
		...
		#ifdef _DEBUG  // jialu
		{
			...
			hFile = CreateFile(TEXT("H:\\Hilbert_signal.bin"), GENERIC_READ | GENERIC_WRITE,FILE_SHARE_READ, 
				NULL, CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL, NULL);	
			WriteFile(hFile,pDst,nLeng*sizeof(float),&dwBytesOfWrite,NULL);
			CloseHandle(hFile);	
		}
		#endif
		//--------------------------------------------------------------------
