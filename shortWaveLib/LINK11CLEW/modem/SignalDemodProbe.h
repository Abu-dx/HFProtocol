...
protected:
	...
	DWORD Demod_16APSK(double *pData,DWORD dwDataLen);

	DWORD Demod_32APSK(double *pData,DWORD dwDataLen);
	...
	DWORD Demod_128QAM(double *pData,DWORD dwDataLen);
...
protected:
	LONG* m_pDemodBuf;				/* 用于存储解调星座图 */

	double* m_pNormalizedDemodBuf;	/* 存储归一化后的星座图数据 */

	DWORD	m_dwDemLen;				/* 输出星座图点数 */

	DWORD	m_dwSylNum;				/* 输出符号个数 */
...
protected:
	...
	DWORD Decode_128QAM(LONG *pData,DWORD dwDataLen);
